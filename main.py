...
class Task(BaseTask):
    ...
    def status(self, value):
        if value in ["Neatlikta", "Vykdoma", "Atlikta"]:
            self._status = value

    def to_dict(self):
        data = asdict(self)
        data["task_type"] = self.get_task_type()
        return data
...
class TaskManager:
    ...
    def edit_task(self, task_id):
        ...
        new_priority = input_colored(
            "Naujas prioritetas (žemas / vidutinis / aukštas): "
        ).lower()
    ...
    def load_from_file(self, filename="tasks.json"):
        try:
            ...
            for i, item in enumerate(data, start=1):
                task_type = item.get("task_type", "paprasta").lower()

                if task_type == "skubi užduotis":
                    task_type = "skubi"
                else:
                    task_type = "paprasta"
                ...
                task = TaskFactory.create_task(
                    ...
                    title=item.get("_title", item.get("title", "Be pavadinimo")),
                    description=item.get("description", "Nėra aprašymo"),
                    priority=item.get("_priority", item.get("priority", "žemas")),
                    deadline=item.get("deadline", "2026-01-01"),
                    status=item.get("_status", item.get("status", "Neatlikta"))
                )
            ...
            self.tasks = []
            print_error("Nepavyko nuskaityti tasks.json failo.")
...
def color_priority(priority):
    ...
    if priority.lower() == "vidutinis":
        return Colors.YELLOW + priority + Colors.END
    if priority.lower() == "žemas":
...
def color_status(status):
    ...
    if status.lower() == "vykdoma":
        return Colors.YELLOW + status + Colors.END
    if status.lower() == "neatlikta":
...
def print_task(task, detailed=False):
    ...
    print(f"{Colors.BOLD}Tipas:{Colors.END}       {task.get_task_type()}")
...
def choose_priority():
    ...
    while True:
        priority = input_colored(
            "Prioritetas (žemas / vidutinis / aukštas): "
        ).lower()
...
def choose_status():
    ...
    while True:
        status = input_colored(
            "Būsena (neatlikta / vykdoma / atlikta): "
        ).lower()
...
def choose_task_type():
    ...
    while True:
        task_type = input_colored("Tipas (paprasta / skubi): ").lower()

        if task_type in types:
            return types[task_type]

        print_error("Pasirinkite: paprasta arba skubi.")
...
def show_menu():
    ...
    print(Colors.CYAN + "12." + Colors.END + " Rūšiuoti pagal prioritetą")
    print(Colors.CYAN + "13." + Colors.END + " Išsaugoti ir išeiti")
